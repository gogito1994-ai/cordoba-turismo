...
9. **Fotos de los otros 14 locales de gastronomía**: búsqueda exhaustiva en Wikimedia Commons, Flickr y Mapillary (julio 2026) sin resultado con licencia libre verificable — la mayoría de fotos encontradas de esos locales tenían copyright "todos los derechos reservados" (incluida la cuenta oficial de Turismo de Córdoba). Mapillary no se pudo explorar visualmente por limitaciones técnicas del entorno; si el propietario quiere intentarlo manualmente, las coordenadas de cada local están en `js/data.js`. Se añadirán si el propietario consigue fotos propias o autorizadas.
